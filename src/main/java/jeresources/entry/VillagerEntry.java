...
public class VillagerEntry extends AbstractVillagerEntry<Villager>{
    ...
    public List<ItemStack> getPois() {
        return VillagersHelper.getPoiBlocks(this.profession.getJobPoiType()).stream().map(blockstate -> new ItemStack(blockstate.getBlock())).collect(Collectors.toList());
    }

    @Override
    public boolean hasPois() {
        return !VillagersHelper.getPoiBlocks(this.profession.getJobPoiType()).isEmpty();
    }
